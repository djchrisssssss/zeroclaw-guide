...
| 類型 | 協定 | Port | 來源 | 用途 |
|------|------|------|------|------|
| SSH | TCP | 22 | 你的 IP（My IP） | SSH 連線（預設必開） |

預設先**只開 SSH**。其餘規則按需再加：

- `42617/tcp`：只有你真的要跑 webhook 時才開，而且建議經反向代理或限制來源。
- `443/tcp`：只有你已經準備好反向代理 / TLS 時才開。
...
### 方法 B：使用 Hardened Systemd Template（生產建議）

以下指令假設你在這個 repo 根目錄內執行：
...
`templates/zeroclaw.service.example` 已經包含 `UMask=0077`、`NoNewPrivileges=yes`、`ProtectSystem=strict`、`ReadWritePaths=%h/.zeroclaw` 等較安全的預設值。
...
預設模板會把 port 綁在 `127.0.0.1:42617`。如果你只用 Telegram polling，通常不需要額外對外開 port。
...
- 預設只開 SSH，且限制為你的 IP
- `443/tcp` 只在你準備好反向代理 / TLS 後開啟
- `42617/tcp` 只在 webhook 模式真的需要時開啟，且不要預設對整個網際網路暴露
- 定期檢查 Security Group 規則

### 12.3 — API Key 安全

- 不要把 API Key 硬編碼在程式碼中
- 優先使用專用 runtime env 檔（`600`）或 AWS Secrets Manager
- 定期輪換 API Key
...
sudo ufw allow ssh
# 如需反向代理: sudo ufw allow 443/tcp
# 如需 webhook gateway: sudo ufw allow 42617/tcp
sudo ufw enable
```
...
需要讀寫的配置檔案用 `600`，私有目錄用 `700`，純讀取的憑證/金鑰用 `400`：
