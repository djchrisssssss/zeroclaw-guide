...
A complete guide for deploying **ZeroClaw AI Agent** on AWS with Telegram integration and agent-level security hardening.
...
┌──────────────────┐  ┌─────────────────────────┐  ┌─────────────────────────┐
│  SETUP_GUIDE     │  │  Security Practice      │  │  Validation Guide       │
│                  │  │  Guide                  │  │                         │
│  AWS + Install   │──│  Red/Yellow line rules  │──│  19 test cases          │
│  Telegram setup  │  │  Permission narrowing   │  │  Verify defenses work   │
│  API config      │  │  13-metric nightly audit│  │                         │
...
ZeroClaw AI Agent 的一站式指南：從 AWS 部署到 Telegram 串接，再到 Agent 安全防護。
...
│  部署教學         │  │  安全實踐指南            │  │  攻防演練手冊            │
│                  │  │                         │  │                         │
│  AWS + 安裝      │──│  紅線/黃線規則           │──│  19 個測試用例           │
│  Telegram 串接   │  │  權限收窄               │  │  驗證防禦是否生效        │
│  API 設定        │  │  每晚巡檢 13 項指標      │  │                         │
│  Daemon 啟動     │  │  Git 災備同步            │  │                         │
└──────────────────┘  └─────────────────────────┘  └─────────────────────────┘
```
